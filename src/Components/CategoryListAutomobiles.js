...
const CategoryListAutomobiles = ({ unfold }) => {
  const styles = {
    art: css`
      display: flex;
  ...
  useEffect(() => {
    fetch(
      "https://api.nytimes.com/svc/topstories/v2/automobiles.json?api-key=8ys3OngIk2CnakCkRcC1vXJApNERITqI"
    )
      .then((response) => {
        if (!response.ok) {
...
export default CategoryListAutomobiles;
